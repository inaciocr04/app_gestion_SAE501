        ...
        <form action="{{ route('student.update', $student->id) }}" method="POST">
            ...
            <div x-show="step === 4" class="step">
                ...
                <div class="flex">
                    ...
                    <x-form.input type="date" name_label="Date de début de la visite" name="start_date_visit" value="{{ old('start_date_visit', isset($visit) && $visit->start_date_visit ? Carbon::parse($visit->start_date_visit)->format('Y-m-d') : '') }}"/>
                    <x-form.input type="date" name_label="Date de fin de la visite" name="end_date_visit" value="{{ old('end_date_visit', isset($visit) && $visit->end_date_visit ? Carbon::parse($visit->end_date_visit)->format('Y-m-d') : '') }}"/>
                </div>
                <p class="flex bg-seventh-color px-6 py-2 rounded-lg mt-4 w-fit cursor-pointer" @click="step--"><x-heroicon-c-arrow-long-left class="w-6 h-auto" /> Précédent</p>
